...
def trend_label(df_tf, tf_name):
    ...
    trend = "多頭" if float(r["Close"]) > (float(r.get("MA5", float(r["Close"]))) or float(r["Close"])) else "空頭"
...
def build_main(df, show_bb):
    ...
    fig.update_layout(
        ...
        height=540, margin=dict(l=52, r=6, t=8, b=6),
...
def build_gauge(value):
    ...
    fig.update_layout(
        ...
        height=175, margin=dict(l=15, r=15, t=28, b=8),
...
def build_macd(df):
    ...
    fig.add_trace(go.Bar(x=df["Date"].tail(40), y=h.tail(40),
        marker_color=[RED if v >= 0 else GRN for v in h.tail(40)], opacity=0.75, name="柱"))
    ...
    fig.update_layout(
        template="plotly_dark", paper_bgcolor=PANEL, plot_bgcolor=BG,
        height=130, margin=dict(l=45, r=6, t=18, b=6),
...
def row(lbl, val, vc=TEXT, bg=CARD):
...
def html_tech(df, ma5, ma20, ma60, kv, dv, rv, mv, sv, hv):
    ...
    if   ma5 > ma20 > ma60: ar,ac = f"多頭排列 (5>20>60)", RED
    elif ma5 < ma20 < ma60: ar,ac = f"空頭排列 (5<20<60)", GRN
    ...
    macd_s = "正值收斂" if mv>0 and hv>0 else ("正值擴張" if mv>0 else
    ...
    return box("".join([
        ...
        row("MA狀態",   ar,     ac),
        row("KD指標",   ks,     kc),
        row("MACD",     macd_s, macd_c),
        row("MACD交叉", cross,  cross_c),
        row("RSI(14)",  rs,     rc),
        row("成交量",   vs,     vc),
        row("量價關係", vp,     vpc),
    ]), "技術分析總覽", "◉")
...
def html_day_script(df, close, atr_v):
    ...
    sc = [
        ("① 開高走高", RED,  f"開盤 > {close:.0f} 量能同步", f"目標 {close+at:.0f} / {close+2*at:.0f}", f"停損 {close-at*.5:.0f}"),
        ("② 盤整整理", GOLD, f"盤在 {close-at*.5:.0f}~{close+at*.5:.0f}", "觀望 等突破方向", f"停損 {close-at:.0f}"),
        ("③ 開低回測", GRN,  f"開盤 < {close:.0f} 觀察量能", f"目標 {close-at:.0f} / {close-2*at:.0f}", f"停損 {close+at*.5:.0f}"),
    ]
    cards = "".join(
...
def run(ticker_input, period_days, show_bb):
    ...
    <div style="background:linear-gradient(90deg,#05080F,#0B1528,#05080F);
      ...
      <div style="display:flex;align-items:center;flex-wrap:wrap;gap:14px;">
        ...
        <div style="display:flex;gap:14px;flex-wrap:wrap;margin-left:auto;">
          {''.join(f'<div style="text-align:center;background:{CARD};border-radius:4px;padding:4px 10px;">'
    ...
    ind_h    = html_industry(sector, industry)
    comp_h   = html_company(name, pe, mktcap)
    board_h  = html_board(ip, ep, iv, ev)
    mf_h     = html_main_force(mf_stat, mf_trend, mf_cc, chip_c, chip_s, mf_col, daily)
    gauge_l  = html_gauge_label(wr)
    levels_h = html_key_levels(daily, close)
    kline_h  = html_kline_patterns(kp, cp)
    multi_h  = html_multiperiod(daily, weekly, monthly)
    vp_h     = html_vol_price(daily)
